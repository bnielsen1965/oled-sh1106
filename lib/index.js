...
class Display {
  ...
  drawPixels(pixels) {
    ...
    pixels.forEach(pixel => {
      ...
      if (pixel[0] >= this.Config.displayColumns || pixel[1] >= this.Config.displayRows) { return; }
  ...
  drawImage(dx, dy, image) {
    ...
    for (let x = 0; x < image.width; x++) {
      dxx = dx + x;
      if (dxx < 0 || dxx >= this.Config.displayColumns) { continue; }
      page = -1; // reset page
      // inner loop through rows
      for (let y = 0; y < image.height; y++) {
        dyy = dy + y;
        if (dyy < 0 || dyy >= this.Config.displayRows) { continue; }
